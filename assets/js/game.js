...
class Game {
    constructor() {
        ...
        this.lose = false
        this.stepCount = 0
        this.word = words[Math.round(Math.random() * words.length)].split('')
        this.displayWord = this.word.map(value => "_")
    }
    ...
    set step(value) {
        ...
        this.gameEvents.dispatch("step_change", {
            "step": this.stepCount
        })
    }

    onLetterPropose(proposedLetter) {
        if (this.win || this.lose || document.activeElement.getAttribute("id") == "proposedWord")
            return

        proposedLetter = proposedLetter.toUpperCase()

        if (authorizedLetter.includes(proposedLetter)) {
            if (!this.player.proposedLetters.includes(proposedLetter)) {
                let containLetter = this.word.includes(proposedLetter)
                this.player.addProposedLetter(proposedLetter)

                if (containLetter) {
                    this.replaceLetter(proposedLetter)
                    this.gameEvents.dispatch("good_letter", {
                        "letter": proposedLetter
                    })
                    this.checkWin()
                } else {
                    this.step++
                    this.gameEvents.dispatch("bad_letter", {
                        "letter": proposedLetter
                    })
                    this.checkLose()
                }
            } else {
                this.gameEvents.dispatch("already_proposed_letter", {
                    "letter": proposedLetter
                })
            }
        } else {
            this.gameEvents.dispatch("not_authorized_letter", {
                "letter": proposedLetter
            })
        }
    }

    onWordPropose(proposedWord) {
        if (this.win || this.lose)
            return

        let word = this.word.reduce((previousValue, currentValue) => `${previousValue}${currentValue}`, "")
        let win = word == proposedWord.toUpperCase()

        proposedWordEl.value = ""

        if (win) {
            this.gameEvents.dispatch("win", {"step": this.step})
        } else {
            this.step++
            this.gameEvents.dispatch("wrong_word_propose", {
                "proposed_word": proposedWord
            })
            this.checkLose()
        }
    }
    ...
    replaceLetter(value) {
        this.word.forEach((letter, index) => {
            if (letter == value)
                this.displayWord[index] = value
        })

        this.gameEvents.dispatch("word_display_change", {
            "new_word": this.displayWord.reduce((previous, current) => `${previous}${current}`, "")
        })
    }

    checkWin() {
        let wordString = this.word.reduce((previousValue, currentValue) => `${previousValue}${currentValue}`, "")
        let wordDisplayedString = this.displayWord.reduce((previousValue, currentValue) => `${previousValue}${currentValue}`, "")

        if (wordString == wordDisplayedString) {
            this.win = true
            this.player.winned()
            this.gameEvents.dispatch("player_win_count_change", {"player": this.player})
            this.gameEvents.dispatch("win", {"step": this.step})
        }
    }
    ...
    checkLose() {
        if (this.step === 7) {
            this.lose = true
            this.player.losed()
            this.gameEvents.dispatch("player_lose_count_change", {"player": this.player})
            this.gameEvents.dispatch("lose")
        }
    }
    ...
    reset() {
        ...
        this.word = words[Math.round(Math.random() * words.length)].split('')
        this.displayWord = this.word.map(value => "_")
        this.player.resetProposedLetters()
        this.gameEvents.dispatch("reset", {"word": word})
    }
}
...
class Player {
    ...
    addProposedLetter(letter) {
        this.letters.push(letter)
        this.game.gameEvents.dispatch("add_proposed_letter", {
            "letter": letter,
            "proposed_letters": this.proposedLetters,
        })
    }

    resetProposedLetters() {
        this.letters = []
    }
